...
import { MOCK_ISSUES, MOCK_EQUIPMENT } from '../constants'; 
...
const IssuesListPage: React.FC = () => {
  ...
  const [filteredIssues, setFilteredIssues] = useState<IssueReport[]>(MOCK_ISSUES);
  ...
  const confirmDelete = () => {
    if(deletingIssueId) {
        ...
        // In real app, MOCK_ISSUES would be updated via API call
        // To reflect deletion in MOCK_ISSUES if it's the source of truth for other parts.
        const mockIndex = MOCK_ISSUES.findIndex(issue => issue.id === deletingIssueId);
        if (mockIndex !== -1) MOCK_ISSUES.splice(mockIndex, 1);
  ...
  const handleFormSubmit = (updatedIssue: IssueReport) => {
    if (editingIssue) {
        ...
        // Update MOCK_ISSUES as well
        const mockIndex = MOCK_ISSUES.findIndex(issue => issue.id === updatedIssue.id);
        if (mockIndex !== -1) MOCK_ISSUES[mockIndex] = updatedIssue;
